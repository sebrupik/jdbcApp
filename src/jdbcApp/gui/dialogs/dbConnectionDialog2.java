...
public class dbConnectionDialog2 extends JDialog {
    ...
    public dbConnectionDialog2(jdbcApp owner, String dbName) {
    ...
    private void initComponents() {
        try {
            serverIP = _owner.getdbConnection2().getDBCO(dbName).attributes[1];
            userName = _owner.getdbConnection2().getDBCO(dbName).attributes[2];
            pwdStr = "";
            ...
            this.setSize(Integer.parseInt(_owner.getSysProperty("sizeX.dbConnectionDialog")), Integer.parseInt(_owner.getSysProperty("sizeY.dbConnectionDialog")));
        ...
        this.addWindowListener(new windowCloser(_owner, conBut));
    ...
    private void connect() {
        ...
        _owner.createdbConnection(dbName, serverAdrTxt.getText(), usrTxt.getText(), String.valueOf(pwdTxt.getPassword()));
        if(!_owner.getdbConnection().isConnected()) {
            ...
            _owner.getdbConnection2().getDBCO(dbName).attributes[1] = serverAdrTxt.getText();
            //_owner.saveSysProperty("jdbc.username", usrTxt.getText());
            _owner.getdbConnection2().getDBCO(dbName).attributes[2] = usrTxt.getText();
